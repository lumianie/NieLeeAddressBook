...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray *)stringArr{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        chineseString.string = [NSString stringWithString:[stringArr objectAtIndex:i]];//取person
...
+(NSMutableArray *)IndexWithArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    for (NSString* object in tempArray)
    {
        NSString *str = ((BMChineseSort *)object).pinYin;//首字母缩写
        NSString *pinyin = [str substringToIndex:1];//第一个首字母
        //不同
        if(![tempString isEqualToString:pinyin])
        {
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    for (BMChineseSort* object in tempArray) {
        ...
        NSString *pinyin = [object.pinYin substringToIndex:1];//首字母
        id obj = object.object;//person对象
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
    ...
    BMChineseSort *BM = tempArray.firstObject;
    if ([[BM.pinYin substringToIndex:1] isEqualToString:@"#"]) {
        NSArray *arr = LetterResult[0];
        [LetterResult removeObjectAtIndex:0];
        [LetterResult addObject:arr];
    }
    return LetterResult;
}
...
+ (NSMutableArray *)ReturnSortObjectArrar:(NSArray*)stringArr key:(NSString *)key{
    ...
    for(int i=0;i<[stringArr count];i++){
        BMChineseSort *chineseString = [[BMChineseSort alloc]init];
        //获取对象对应字段的字符串
        id temp = [stringArr objectAtIndex:i];//对象
       // NSLog(@"%@", [temp valueForKeyPath:key]);
        chineseString.string = [NSString stringWithString:[temp valueForKeyPath:key]];//姓名
        ...
        //这里我自己写了一个递归过滤指定字符串   RemoveSpecialCharacter
        chineseString.string = [BMChineseSort RemoveSpecialCharacter:chineseString.string];//去除特殊字符
        // NSLog(@"string====%@",chineseString.string);
        
        //判断首字符是否为字母
        NSString *regex = @"[A-Za-z]+";
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
        NSString *initialStr = [chineseString.string length]?[chineseString.string substringToIndex:1]:@"";//取首字
        if ([predicate evaluateWithObject:initialStr])//是字母
        {
            NSLog(@"chineseString.string== %@",chineseString.string);
            //首字母大写
            chineseString.pinYin = [chineseString.string capitalizedString] ;
        }else{//不是字母
            if(![chineseString.string isEqualToString:@""]){//姓名非 空和特殊字母
                NSString *pinYinResult = [NSString string];
                for(int j=0;j<chineseString.string.length;j++){//将汉字转化成大写首字母
                    NSString *singlePinyinLetter = [[NSString stringWithFormat:@"%c",
    ...
    //        NSLog(@"chineseStringsArray====%@",((ChineseString*)[chineseStringsArray objectAtIndex:i]).pinYin);
    //    }
//    BMChineseSort *BM = chineseStringsArray.firstObject;
//    if ([[BM.pinYin substringToIndex:1] isEqualToString:@"#"]) {
//        BMChineseSort *temp = BM;
//        [chineseStringsArray removeObjectAtIndex:0];
//        [chineseStringsArray addObject:temp];
//    }
        return chineseStringsArray;
}
